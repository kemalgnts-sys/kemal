...
def test_repeating_completed_step_does_not_advance_progress_twice(monkeypatch):
    progress = make_progress(current_step=0)
    progress["steps"][0]["completed"] = False
    progress["steps"][1]["completed"] = False
    fake_db = FakeDB(inspection_progress=[progress])
    monkeypatch.setattr(server, "db", fake_db)

    asyncio.run(server.complete_step("inspection-1", "exterior_front", notes="front ok"))
    asyncio.run(server.complete_step("inspection-1", "exterior_front", notes="front ok"))

    assert fake_db.inspection_progress.docs[0]["current_step"] == 1
    assert fake_db.inspection_progress.docs[0]["steps"][0]["notes"] == "front ok"


def test_upload_photo_rejects_invalid_step_and_sanitizes_filename(monkeypatch, tmp_path):
    fake_db = FakeDB(inspection_progress=[make_progress()])
    monkeypatch.setattr(server, "db", fake_db)
    monkeypatch.setattr(server, "UPLOADS_DIR", tmp_path)

    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(server.upload_inspection_photo(
            "inspection-1",
            "../../outside",
            SimpleNamespace(filename="car.jpg", file=io.BytesIO(b"fake")),
        ))
    assert exc_info.value.status_code == 404

    response = asyncio.run(server.upload_inspection_photo(
        "inspection-1",
        "exterior_front",
        SimpleNamespace(filename="car.jpg/../../outside", file=io.BytesIO(b"fake")),
    ))

    uploaded_name = response["photo_url"].removeprefix("/uploads/")
    assert "/" not in uploaded_name
    assert ".." not in uploaded_name
    assert (tmp_path / uploaded_name).exists()


def test_submit_report_rejects_premature_submission(monkeypatch):
    fake_db = FakeDB(inspections=[make_inspection(status="accepted", inspector_id="inspector-1")])
    monkeypatch.setattr(server, "db", fake_db)
