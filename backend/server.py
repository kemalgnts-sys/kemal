...
async def get_verified_inspector(inspector_id: str) -> dict:
    inspector = await db.users.find_one({"id": inspector_id}, {"_id": 0})
    if not inspector:
        raise HTTPException(status_code=404, detail="Inspector not found")
    if inspector.get("user_type") != "inspector":
        raise HTTPException(status_code=403, detail="User is not an inspector")
    
    profile = await db.inspector_profiles.find_one({"user_id": inspector_id}, {"_id": 0})
    if not profile or not profile.get("id_verified"):
        raise HTTPException(status_code=403, detail="Inspector must be verified")
    
    return inspector

def build_progress_doc(inspection_id: str) -> dict:
...
async def get_available_inspections(inspector_lat: float = 41.8781, inspector_lng: float = -87.6298, radius: int = 50):
    ...
    for inspection in inspections:
        ...
        if distance <= radius:
            safe_inspection = inspector_safe_inspection(inspection)
...
async def accept_inspection(inspection_id: str, inspector_id: str):
    inspector = await get_verified_inspector(inspector_id)
...
async def verify_security_code(inspection_id: str, code: str, inspector_id: str):
    inspection = await db.inspections.find_one({"id": inspection_id}, {"_id": 0})
    if not inspection:
        raise HTTPException(status_code=404, detail="Inspection not found")
    await get_verified_inspector(inspector_id)
    
    if inspection.get("inspector_id") != inspector_id:
        raise HTTPException(status_code=403, detail="Inspection is assigned to another inspector")
    ...
    if inspection["status"] == "in_progress":
        ...
        if not progress:
            await db.inspection_progress.insert_one(build_progress_doc(inspection_id))
        return {"message": "Code already verified", "steps": INSPECTION_STEPS}
    if inspection["status"] != "accepted":
        raise HTTPException(status_code=400, detail="Inspection is not ready to start")
...
async def submit_report(inspection_id: str, report: InspectionReportCreate, inspector_id: str):
    ...
    update_result = await db.inspections.update_one(
        {"id": inspection_id, "status": "in_progress", "inspector_id": inspector_id},
...
async def get_inspector_jobs(inspector_id: str):
    ...
    return [inspector_safe_inspection(job) for job in jobs]
