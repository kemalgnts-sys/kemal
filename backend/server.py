...
ALLOWED_INSPECTION_STEP_NAMES = {step["step_name"] for step in INSPECTION_STEPS}
SAFE_UPLOAD_EXTENSIONS = {"jpg", "jpeg", "png", "webp", "heic", "heif"}

def without_security_code(inspection: dict) -> dict:
    sanitized = dict(inspection)
    sanitized.pop("security_code", None)
    return sanitized

def update_modified(result) -> bool:
...
async def accept_inspection(inspection_id: str, inspector_id: str):
    ...
    if not update_modified(update_result):
        raise HTTPException(status_code=400, detail="Inspection already accepted")
    ...
    return without_security_code(updated)
...
async def verify_security_code(inspection_id: str, code: str):
    ...
    if not update_modified(update_result) and inspection["status"] != "in_progress":
        raise HTTPException(status_code=400, detail="Inspection is not ready for code verification")
...
async def upload_inspection_photo(
    ...
    if step_name not in ALLOWED_INSPECTION_STEP_NAMES:
    ...
    # Save file using server-generated path components only.
    file_ext = Path(file.filename).suffix.lower().lstrip(".") if file.filename else "jpg"
    if not file_ext:
        file_ext = "jpg"
...
async def complete_step(inspection_id: str, step_name: str, notes: str = ""):
    ...
    step_index = next((idx for idx, step in enumerate(progress["steps"]) if step["step_name"] == step_name), None)
    ...
    if step_index != current:
        raise HTTPException(status_code=400, detail="Complete the current inspection step first")
...
async def submit_report(inspection_id: str, report: InspectionReportCreate):
    ...
    # Claim completion before side effects so retries cannot duplicate payouts.
    update_result = await db.inspections.update_one(
